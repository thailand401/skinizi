...
$(document).ready(function(){
	...
	$("#addtocart").click(function(){
		...
		if($("#checkterms").attr("checked") == 'checked')
		{
			...
			var xi,yi,isi = 0;
			for(yi = 0; yi < layerArt.length; yi++){
			    for(xi = 0; xi < layerArt[yi].list.length; xi++)
			    	if(layerArt[yi].list[xi].type == 'image'){
			    		isi++;
			    		break;
			    	}
		    }
		    if (isi < layerArt.length) {alert('Other layer dont have image');return;};
			...
			$.post("execute", { data:$("#vdata").serialize() + " " + $("#ddata").serialize() },
			    function(data){
			    	if(count > 0){
			    		...
			    		//$('.btnright').click();
			   			$('#review a').attr('href','review/index/Y'+$('#review a').attr('href')+"/"+data);
			   			count = 1;
		...
		{
			alert("Please check Agree to terms & conditions");
		}
	...
	$('#waiting').css({'top': $('.canvas-container').position().top, 'left': $('.canvas-container').position().left, 'width': $('.canvas-container').css('width'), 'height': $('.canvas-container').css('height')});
...
function RenderAgain(){
	...
	big.set({ left: big.width/2 + 188 - nx - 15, top: big.height/2 + 39});
